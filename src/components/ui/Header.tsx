...
export function Header() {
  ...
  useEffect(() => {
    const handlePreloader = () => setIsPreloaderFinished(true);
    window.addEventListener("preloaderFinished", handlePreloader);
    
    // Fallback just in case
    const timeout = setTimeout(() => setIsPreloaderFinished(true), 4500);
    
    return () => {
      window.removeEventListener("preloaderFinished", handlePreloader);
        ...
        >
          <div 
            ...
            className={`w-full max-w-5xl flex items-center justify-between p-2 rounded-full border transition-all duration-500 ease-out ${
              isScrolled 
                ? "bg-[#0A0A0A]/70 backdrop-blur-md border-[rgba(255,107,0,0.1)] shadow-[0_8px_32px_rgba(255,107,0,0.1)]" 
          ...
          >
            ...
            >
              <div className="w-9 h-9 rounded-full bg-[#ff6b00] flex items-center justify-center transition-transform duration-500 group-hover:rotate-12 shadow-[0_0_15px_rgba(255,107,0,0.5)]">
              ...
              <div className="hidden sm:flex flex-col relative overflow-hidden h-8 justify-center">
                <span className="text-white font-heading font-black text-base leading-none transform group-hover:-translate-y-[150%] transition-transform duration-300">Mahesh.</span>
            ...
            <nav className="hidden lg:flex items-center gap-1 bg-white/5 rounded-full px-2 py-1 border border-white/5 relative">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  className={`relative px-4 py-2 text-[10px] font-mono font-bold tracking-widest uppercase transition-all duration-300 rounded-full group overflow-hidden ${
                    activeSection === link.href.slice(1) 
                      ? "text-[#ff6b00]" 
                      : "text-white/60 hover:text-white"
                  }`}
                >
                  <span className="relative z-10 group-hover:-translate-y-[1px] transition-transform inline-block">{link.name}</span>
                  {/* Hover effect - smooth underline growth & orange glow */}
                  <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-[2px] bg-[#ff6b00] rounded-full transition-all duration-300 group-hover:w-3/4 group-hover:shadow-[0_0_8px_#ff6b00]"></span>
            ...
            {/* Action Widgets */}
            <motion.div 
              className="hidden lg:flex items-center gap-3 pr-2"
            ...
            >
              ...
              <a 
                ref={magneticRef}
                href="#contact" 
                className="flex items-center gap-1.5 px-5 py-2.5 bg-white text-black hover:bg-[#ff6b00] hover:text-black text-[10px] font-mono font-black tracking-widest uppercase rounded-full transition-colors duration-300 group shadow-[0_0_0_rgba(255,107,0,0)] hover:shadow-[0_0_20px_rgba(255,107,0,0.4)]"
              >
                Connect
